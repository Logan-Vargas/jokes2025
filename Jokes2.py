# Joke Creation Tool (refactoring the bad performance task) 
# By Antonio Williams and Logan Vargas

system_on = True
joke_tag = []
joke_punch = []

# Final Phase - User rates the game and gives recommendation
def final_phase():
...
# Here, the user is prompted to create a joke by naming it and providing a punchline.
...
# Here, the program will display the names of the jokes, and will move on if none are created! The user can select which joke they want.
    ...
    print(joke_tag)
    friend_input = str(input("Which joke do you want? Type 'end' to proceed to the next phase. ")) # User selects which joke they want to hear.
